...
function mapFormData(formData) {
  ...
  const month = formData.get("birth-month").padStart(2, "0");
  const day = formData.get("birth-day").padStart(2, "0");
  mappedData.set("et_pb_contact_fecha_nacimiento_0", `${year}-${month}-${day}`);

  // Format the phone number (remove non-numeric characters)
  const phone = formData.get("et_pb_contact_phone_0").replace(/\D/g, "");
  mappedData.set("et_pb_contact_phone_0", phone);

  // Ensure research areas are set correctly
  const researchAreas = [
    "Ciencias Naturales",
  ...
  researchAreas.forEach((area, index) => {
    if (
      formData.get(`research-area`) === area.toLowerCase().replace(/ /g, "-")
    ) {
      mappedData.set(`et_pb_contact_area_investigacion_0_23_${index}`, area);
    }
  });
...
function prepareFormData() {
  ...
  formData.set("et_pb_contact_fecha_ingreso_0", getCurrentDate());

  // Set CAPTCHA
  const captchaChallenge = document.cookie
    .split("; ")
    .find((row) => row.startsWith("captcha_text="))
    ?.split("=")[1];
  if (captchaChallenge) {
    formData.set("captcha_challenge", captchaChallenge);
  }
...
function submitForm(event) {
  ...
  fetch(
    ...
    .then((result) => {
      ...
      if (result.includes("Error en datos de ingreso")) {
      ...
      } else if (result.includes("Location:")) {
