...
import('./test-utils.js')
  .then(() => console.log('Test utilities loaded successfully'))
  .catch(err => console.log('Test utilities not available in this environment'));
...
const ROLE_MAPPINGS = {
  ...
  "tecnico": "personal tecnico",
  "consultor": "consultor_asesor"
};
...
function prepareFormData(rawFormData) {
  ...
  // UC5: Map the role value before sending
  const originalRole = formData.get('et_pb_contact_rol_0');
  if (originalRole && ROLE_MAPPINGS[originalRole]) {
    formData.set('et_pb_contact_rol_0', ROLE_MAPPINGS[originalRole]);
    console.log('Role mapping:', {
      original: originalRole,
      mapped: ROLE_MAPPINGS[originalRole]
    });
  } else {
    console.warn('Role not mapped:', originalRole);
  }
