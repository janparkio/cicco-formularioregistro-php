...
function handleResearchAreas(formData) {
  // Get array of selected research area IDs by checking which checkboxes are checked
  const selectedAreas = Object.keys(RESEARCH_AREAS).filter(id => document.getElementById(id)?.checked);

  // Validate that at least one research area is selected
  ...
  // Process each selected research area
  selectedAreas.forEach(areaKey => {
    const {id, value, pythonKey} = RESEARCH_AREAS[areaKey];
    // Add values for both the form submission and Python backend processing
    formData.append(`et_pb_contact_area_investigacion_0_23_${id}`, value); // Form field
    formData.append(pythonKey, value); // Python backend field
  });

  // Flag that research areas were provided
...
function prepareFormData(rawFormData) {
  ...
  // Map form fields and log each mapping
  Object.entries(FIELD_MAPPINGS).forEach(([rawField, serverField]) => {
    const value = rawFormData.get(rawField);
    formData.append(serverField, value);
    console.log(`Mapping ${rawField} -> ${serverField}:`, value);
  });

  // Construct and add birth date in YYYY-MM-DD format
  ...
  console.log('Research areas processed');

  // Handle captcha token
  const captchaToken = document.getElementById("captcha_token")?.value;
  if (captchaToken) {
    formData.append("captcha_token", captchaToken);
    console.log("Appending captcha token:", captchaToken);
  } else {
    console.warn("Captcha token not found in form");
  }
...
async function handleFormSubmission(form, formData) {
  ...
  try {
    ...
    if (result.debug) {
      ...
      console.log('Raw Form Data:', result.debug.formData);
