...
function handleResearchAreas(formData) {
  ...
  selectedAreas.forEach(areaKey => {
    ...
    formData.append(serverField, area.serverValue);
    
    console.log('Processing area:', {
      key: areaKey,
      value: area.value,
      serverValue: area.serverValue,
  ...
  const joinedValue = selectedAreas
    .map(key => RESEARCH_AREAS[key].serverValue)
    .join(',');
  formData.append('et_pb_contact_area_investigacion_0', joinedValue);

  // Ensure all research area fields are set, even if not selected
  Object.values(RESEARCH_AREAS).forEach(area => {
    const serverField = `et_pb_contact_area_investigacion_0_23_${area.id}`;
    if (!formData.has(serverField)) {
      formData.append(serverField, ''); // Send empty string instead of "null"
    }
  });
  ...
  try {
    ...
    const attemptData = {
      ...
      request_data: {
        ...
        ingenieria_y_tecnologia: formDataLog.et_pb_contact_area_investigacion_0_23_1 || "",
        ciencias_medicas_y_de_la_salud: formDataLog.et_pb_contact_area_investigacion_0_23_2 || "",
        ciencias_agricolas: formDataLog.et_pb_contact_area_investigacion_0_23_3 || "",
        ciencias_sociales: formDataLog.et_pb_contact_area_investigacion_0_23_4 || "",
        humanidades: formDataLog.et_pb_contact_area_investigacion_0_23_5 || ""
      },
      response: {
        success: result.success,
