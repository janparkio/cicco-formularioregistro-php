  ...
  Object.entries(FIELD_MAPPINGS).forEach(([clientField, serverField]) => {
    ...
    if (value?.trim()) {
      let sanitizedValue = sanitizeValue(value, {
        ...
        trimEnd: true,
        cleanCommasAtEnds: true,
        normalizeSpanish: ['organizacion', 'et_pb_contact_nombres_0', 'et_pb_contact_apellidos_0'].includes(serverField)
      });

      // Asegurarse que la institución coincida exactamente con la cookie
