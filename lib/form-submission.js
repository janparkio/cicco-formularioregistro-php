...
function mapFormData(formData) {
  ...
  researchAreas.forEach((area, index) => {
    mappedData.append(
      ...
      area === "ciencias-naturales" ? "Ciencias Naturales" : area,
    );
  });
...
function prepareFormData() {
  ...
  const captchaChallenge = document.cookie.replace(
    /(?:(?:^|.*;\s*)captcha_text\s*\=\s*([^;]*).*$)|^.*$/,
    "$1",
  );
  if (captchaChallenge) {
...
function setInstitutionCookies(institution, faculty) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (form) {
    ...
    if (institutionNameSearch && campusFaculty) {
      institutionNameSearch.addEventListener("change", function () {
        const institution = this.value;
        const faculty = campusFaculty.value;
        setInstitutionCookies(institution, faculty);
      });

      campusFaculty.addEventListener("change", function () {
        const institution = institutionNameSearch.value;
