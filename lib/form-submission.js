    ...
    .then(text => {
        try {
            ...
            if (data.status === 'success') {
                ...
                showErrorMessage(data.message || 'Error al procesar el formulario');
            }
        } catch (e) {
            // Handle non-JSON response (like redirects)
            if (text.includes('Location:')) {
                const url = text.match(/Location: ([^\n]+)/)[1];
                window.location.href = url;
            } else {
                showErrorMessage('Error inesperado al procesar la respuesta');
            }
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('Error al enviar el formulario. Por favor, inténtelo de nuevo.');
    })
    .finally(() => {
        if (loadingIndicator && submitButton) {
