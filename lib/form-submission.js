...
function handleResearchAreas(formData) {
  ...
  // Agregar cada área como un campo separado
  selectedAreas.forEach(areaKey => {
    const value = sanitizeValue(RESEARCH_AREAS[areaKey].value, {
      cleanCommasAtEnds: true,
      trimStart: true,
      trimEnd: true,
      normalizeSpanish: true
    });
    const serverField = `et_pb_contact_area_investigacion_0_23_${RESEARCH_AREAS[areaKey].id}`;
    formData.append(serverField, value);
  });

  // También mantener el campo concatenado para compatibilidad
  const joinedValue = selectedAreas.map(key => RESEARCH_AREAS[key].value).join(',');
  formData.append('et_pb_contact_area_investigacion_0', joinedValue);
  
  console.log('Research areas added:', Object.fromEntries(formData));
  return { isValid: true };
}
  ...
  Object.entries(FIELD_MAPPINGS).forEach(([clientField, serverField]) => {
    ...
    if (value?.trim()) {
      ...
      if (serverField === 'organizacion') {
        ...
        if (orgCookie) {
          // Decodificar y obtener el valor exacto de la cookie
          sanitizedValue = decodeURIComponent(orgCookie)
            .replace(/\\"/g, '')    // Remover \" escapadas
            .replace(/"/g, '')      // Remover comillas dobles normales
            .replace(/\\\\/g, '')   // Remover dobles backslashes
            .replace(/\\'/g, "'")   // Convertir \' a '
            .replace(/\\n/g, '')    // Remover saltos de línea escapados
            .trim();                // Limpiar espacios extras

          // Debug log para ver el valor exacto
          console.log('Cookie original:', orgCookie);
          console.log('Cookie procesada:', sanitizedValue);
        }
      } else {
  ...
  try {
    ...
    const attemptData = {
      ...
      request_data: {
        ...
        ingenieria_y_tecnologia: formDataLog.et_pb_contact_area_investigacion_0_23_1 || null,
        ciencias_medicas_y_de_la_salud: formDataLog.et_pb_contact_area_investigacion_0_23_2 || null,
        ciencias_agricolas: formDataLog.et_pb_contact_area_investigacion_0_23_3 || null,
        ciencias_sociales: formDataLog.et_pb_contact_area_investigacion_0_23_4 || "",
        humanidades: formDataLog.et_pb_contact_area_investigacion_0_23_5 || null
      },
      response: {
        success: result.success,
