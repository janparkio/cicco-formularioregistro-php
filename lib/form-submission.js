...
function prepareFormData(rawFormData) {
    ...
    // Fix: Update captcha token field name to match server expectation
    const captchaToken = document.getElementById("captcha_token")?.value;
    if (captchaToken) {
        formData.append("captcha_token", captchaToken);
        console.log('Appending captcha token:', captchaToken); // Debug log
    } else {
        console.warn('Captcha token not found in form'); // Debug warning
    }
...
//       "https://cicco.conacyt.gov.py/solicitud_registro_usuario/lib/captcha.php",
//       {
//         method: 'GET',
//         credentials: 'include',
//         headers: {
...
//       console.log('CAPTCHA Token successfully set');
//     }
//   } catch (error) {
...
async function loadCaptcha() {
  ...
  const captchaPath = '/solicitud_registro_usuario/lib/captcha.php';

  try {
    // Add timestamp to prevent caching
    captchaImg.src = captchaPath + '?' + new Date().getTime();
    captchaInput.value = '';
    
    // Verify CAPTCHA script is accessible
    const response = await fetch(captchaPath, {
      credentials: 'include',
      headers: {
    ...
    showMessage('error', 'Error al cargar el CAPTCHA. Por favor, recargue la página.');
  }
}
...
async function handleFormSubmission(form, formData) {
  ...
  if (!captchaInput?.value) {
    ...
    console.error('CAPTCHA input not found or empty');
    return;
  }

  // Ensure CAPTCHA value is included in form data
  ...
  try {
    ...
    if (!result.success) {
      if (result.message?.includes('CAPTCHA')) {
        ...
        displayErrorMessages(result.errors || [{ message: result.message || 'Unknown error' }]);
      }
      throw new Error(result.message || 'Error desconocido en el procesamiento del formulario.');
    }
    ...
    showMessage('error', error.message || 'Error al enviar el formulario. Por favor, intente nuevamente.');
  } finally {
...
function validateField(fieldId, errorMessage) {
  ...
  if (fieldId === 'id-number') {
    ...
    errorMessage = isValid ? '' : 'El número de cédula debe tener entre 6 y 15 dígitos';
  }
