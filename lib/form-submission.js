...
function prepareFormData(rawFormData) {
  ...
  const areaMapping = {
    ...
    "ingenieria-tecnologia": "1",
    "ciencias-medicas-salud": "2",
    "ciencias-agricolas-veterinarias": "3",
    "ciencias-sociales": "4",
    "humanidades-artes": "5",
    "otras-areas": "0", // Map "otras areas" to Ciencias Naturales as fallback
  };

  researchAreaIds.forEach((id) => {
    const checkbox = document.getElementById(id);
    if (checkbox && checkbox.checked) {
      const mappedId = areaMapping[id];
      formData.append(
        `et_pb_contact_area_investigacion_0_23_${mappedId}`,
        checkbox.value
      );
    }
  });
...
function validateForm(formData) {
  ...
  // Check if at least one research area is selected
  const hasResearchArea = researchAreaIds.some(id =>
  ...
  if (!hasResearchArea) {
    ...
    // Add selected areas to form data
    researchAreaIds.forEach(id => {
      const checkbox = document.getElementById(id);
      if (checkbox?.checked) {
        const mappedId = areaMapping[id];
