...
function handleResearchAreas(formData) {
  ...
  // Collect and sanitize selected values
  const areaValues = selectedAreas.map(areaKey => 
    sanitizeValue(RESEARCH_AREAS[areaKey].value, {
      cleanCommasAtEnds: true,
      trimStart: true,
      trimEnd: true,
      normalizeSpanish: true // Activar normalización de caracteres especiales para áreas de investigación en español.
    })
  );
  
  // Join the values preserving internal spaces and commas
  const joinedValue = areaValues.join(',');
  formData.append('et_pb_contact_area_investigacion_0', joinedValue);
  console.log(`Adding research areas: ${joinedValue}`);
...
function prepareFormData(rawFormData) {
  ...
  Object.entries(FIELD_MAPPINGS).forEach(([clientField, serverField]) => {
    ...
    if (value?.trim()) {
      // Sanitize all values before adding to formData
      const sanitizedValue = sanitizeValue(value, {
        trimStart: true,
        trimEnd: true,
        cleanCommasAtEnds: true,
        normalizeSpanish: ['organizacion', 'et_pb_contact_nombres_0', 'et_pb_contact_apellidos_0'].includes(serverField) // Normalize Spanish text for names and institution
      });
