...
async function fetchCaptchaToken() {
  try {
    console.group('CAPTCHA Token Generation');
    ...
    if (data.token) {
      ...
      if (tokenInput) {
        ...
        document.cookie = `captcha_token=${data.token}; path=/; domain=.conacyt.gov.py; SameSite=Lax; Secure`;
...
async function handleFormSubmission(form, formData) {
  ...
  console.log('Captcha Token:', formData.get('captcha_token'));
  ...
  try {
    ...
    try {
      ...
      console.log('Invalid Response Text:', responseText);
    ...
    if (!result.success) {
      console.error('Server Error:', result.debug || 'No debug info available');
    ...
    if (error.name === 'TypeError') {
      showErrorMessage('Error de conexión. Por favor, verifique su conexión a internet.');
    } else {
      showErrorMessage(`Error en el envío: ${error.message}. Por favor, contacte al soporte técnico para obtener ayuda.`);
    }
